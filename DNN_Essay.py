    ...
    for i in range(1,len(structure)+1):
        with tf.variable_scope('clayer{}'.format(i),reuse=reuse):
            filter_weight = tf.get_variable(
                'weights',structure[i]['conv']['filter'],initializer=tf.truncated_normal_initializer(mean=0.1,stddev=0.1,seed=1)
            )
            biases = tf.get_variable(
...
if __name__ == '__main__':
    ...
    conv_structure = {
        1:{
            'conv':{'filter':[3,3,1,6],'stride':[1,1,1,1],'padding':'SAME'},
    ...
    y = full_connected(reshaped, hidden_layer, regularizer)
    cost = tf.reduce_mean(
    ...
    learning_rate = tf.train.exponential_decay(learning_rate_base,global_step,stair_num,learning_rate_decay,staircase=True)
